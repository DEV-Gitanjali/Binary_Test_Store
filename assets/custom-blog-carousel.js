class BlogCarousel extends HTMLElement {
  connectedCallback() {
    this.list = this.querySelector('.blog-carousel__list');
    if (!this.list) return;

    this.items = Array.from(this.list.children);
    this.prevBtn = this.querySelector('.blog-carousel__nav-btn--prev');
    this.nextBtn = this.querySelector('.blog-carousel__nav-btn--next');
    this.dots = Array.from(this.querySelectorAll('.blog-carousel__dot'));
    this.isStatic = this.list.classList.contains('blog-carousel__list--static');
  ...
  scrollByCard(direction) {
    const item = this.list.querySelector('.blog-carousel__item');
    if (!item) return;
